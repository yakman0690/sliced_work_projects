...
public enum TypeChargeOn {
    ...
    public static TypeChargeOn fromValue(String v) {
        Logger logger = LoggerFactory.getLogger(TypeChargeOn.class);
         for (TypeChargeOn c: TypeChargeOn.values()) {
            //logger.info("value is {}",c.value);
             if (c.value.equals(v)) {
                 return c;
             }
         }
     ...
     public static TypeChargeOn fromName(String v) {
        v = v.replace("\"", "");
        Logger logger = LoggerFactory.getLogger(TypeWriteOff.class);
        for (TypeChargeOn c: TypeChargeOn.values()) {
            if (c.name().equals(v)) {
                return c;
            }
